...
function parseUrlLength(): number {
  const lengthParam = urlParam("count");
...
function parseUrlGameNumber(): number {
  const gameParam = urlParam("match");
...
function Game(props: GameProps) {
  ...
  const currentSeedParams = () =>
    `?today=${seed}&count=${wordLength}&match=${gameNumber}`;
  useEffect(() => {
    if (seed) {
    ...
    <div className="Game" style={{ display: props.hidden ? "none" : "block" }}>
      ...
      <div className="Game-seed-info">
        {challenge
          ? "playing a summons"
          : seed
          ? `${describeSeed(seed)} — symbol count ${wordLength}, game ${gameNumber}`
          : "playing a random match"}
      </div>
      <p>
        <button
          onClick={() => {
            share("Summons link now on clipboard!");
          }}
        >
          Summon a pal to this match
        </button>{" "}
        {gameState !== GameState.Playing && (
          <button
            onClick={() => {
